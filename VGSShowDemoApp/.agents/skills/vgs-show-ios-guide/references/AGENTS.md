...
- The canonical durable instruction source for this repository lives at `skills/vgs-show-ios-guide/references/AGENTS.md`.
- The single public AI skill entrypoint lives at `skills/vgs-show-ios-guide/SKILL.md`. It routes task type and version selection, then defers SDK rules and invariants to this file.
- The installed skill bundle must ship `skills/vgs-show-ios-guide/references/AGENTS.md` so standalone skill installs receive this policy file automatically.
- For the public skill and public documentation workflows, the canonical repository for tags and versioned docs is `https://github.com/verygoodsecurity/vgs-show-ios`.
- When the SDK version used by the target project can be determined, agents MUST prefer the durable guidance snapshot from the matching Git tag in the public `verygoodsecurity/vgs-show-ios` repository before giving version-sensitive guidance. For older tags, that snapshot may still live at repo-root `AGENTS.md`.
...
- Release tags MUST be created only after `README.md`, `skills/vgs-show-ios-guide/SKILL.md`, and `skills/vgs-show-ios-guide/references/AGENTS.md` match the shipped SDK behavior for that release.
...
8. Unsubscribe logic used to prevent stale views or retain cycles when views deallocate.
9. UI masking (`isSecureText` or secure ranges) applied before any user-visible refresh when required.
...
- Don't derive environment from user input; use build config or secure remote config with an allow-list.
...
3. Configure mask, placeholder, or delegate.
...
- If no subscribed views, a warning is logged; enforce a precondition if unintended.
...
- Single `VGSShow` instance per logical screen or context.
- Batch multiple view reveals in one request.
- Avoid overlapping multiple requests for the same view set; serialize unless streaming updates are required.
- Large base64 assets: ensure the backend enforces size caps; consider progress indicators for large PDFs (future enhancement - current API is synchronous post-decode to UI).
...
6. Copy Operation: With transformation + masking, copy `.transformed` vs `.raw`; ensure clipboard content matches expected format and never includes the masked symbol when copying raw intentionally (only if policy allows raw copy).
...
[ ] Tests updated or green (see §16).

---
## 19. Common Failure Modes & Mitigations
| Failure | Cause | Mitigation |
|---------|-------|-----------|
| Warning: empty content path | Forgot to set `contentPath` | Add pre-request assert or factory guard |
...
4. Optionally set `customHeaders` and build a payload listing the necessary keys.
5. Invoke a single batched `request`.
6. On callback success: UI is already updated; adjust mask ranges if needed.
